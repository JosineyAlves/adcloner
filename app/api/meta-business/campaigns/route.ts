  ...
  try {
    ...
    try {
      ...
      if (campaignsData.data && campaignsData.data.length > 0) {
        ...
        // campanha(s) do que campanhas, entao ha mais lotes e mais sub-requests individuais que
        ...
        if (failedInsightsIndexes.length > 0) {
          console.warn(`⚠️ ${failedInsightsIndexes.length} campanha(s) com erro no lote de insights, tentando novamente...`)
          const retryIds = failedInsightsIndexes.map((idx) => campaignIds[idx])
          await new Promise((resolve) => setTimeout(resolve, 500))
          const retryBatch = facebookBatchAPI.createCampaignInsightsBatch(retryIds, datePreset, since || undefined, until || undefined, metricIds)
          ...
          if (stillFailingCount > 0) {
            console.warn(`⚠️ ${stillFailingCount} campanha(s) continuam com erro apos nova tentativa — serao exibidos com metricas zeradas.`)
          }
        }
