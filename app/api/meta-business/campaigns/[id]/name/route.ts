...
export async function POST(
  ...
  try {
    ...
    console.log(`🔄 Atualizando nome da campanha ${campaignId}:`, { name: trimmedName })

    // Atualizar nome usando POST /{id} com o campo "name" — mesmo endpoint e padrão usados para
    // status e orçamento (ver campaigns/[id]/budget/route.ts), coberto pela mesma permissão
    // ads_management já solicitada no App Review — não exige nenhuma permissão adicional da Meta.
    ...
    // quando o status HTTP já é 4xx — o parse do corpo tem que acontecer ANTES de decidir se
    // foi erro (mesmo padrão de app/api/meta-business/campaigns/[id]/budget/route.ts).
    ...
    console.log(`✅ Nome da campanha ${campaignId} atualizado com sucesso`)
