...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Chamada para Facebook Marketing API para atualizar status do anúncio
    ...
    if (!facebookResponse.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to update ad status', details: errorData },
    ...
    return NextResponse.json({
      success: true,
      message: `Ad ${status === 'ACTIVE' ? 'activated' : 'paused'} successfully`,
      ad_id: id,
      status: status
    })
