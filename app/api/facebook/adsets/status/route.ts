...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Chamada para Facebook Marketing API para atualizar status do ad set
    ...
    if (!facebookResponse.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to update ad set status', details: errorData },
    ...
    return NextResponse.json({
      success: true,
      message: `Ad set ${status === 'ACTIVE' ? 'activated' : 'paused'} successfully`,
      adset_id: id,
      status: status
    })
